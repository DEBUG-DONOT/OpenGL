...
int main()
{
    ...
    std::string pathPBR = "Resourse/Cerberus_by_Andrew_Maximov/Cerberus_LP.FBX";
    //////////////////////////////////////////////////////
    GameObject go;
    auto* x = go.AddComponent<ModelComponent>();
    //x->LoadModel(path);
    x->LoadModel(pathPBR);
    x->ShowAllTextureType();
    go.AddComponent<TransformComponent>();
    auto tsCop=dynamic_cast<TransformComponent*>( go.GetComponentByName("Transform"));
    tsCop->SetPosition(glm::vec3(0, 10, 100));
    ...
    VertexShader v("Resourse/ShaderLib/PBRVert.glsl");
    FragmentShader f("Resourse/ShaderLib/PBRFrag.glsl");
    Shader s(v, f); 
